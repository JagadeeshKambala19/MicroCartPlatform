...
app.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const existing = await query(
      "SELECT id, quantity FROM cart WHERE product_id=:productId",
      { productId }
    );
    ...
    if (existing.length) {
      ...
      await query(
        ...
        { id: existing[0].id, quantity: nextQty }
      );

      const rows = await query(
        "SELECT id, product_id, quantity FROM cart WHERE id=:id",
        { id: existing[0].id }
      );
...
const server = app.listen(PORT, "0.0.0.0", () => {
  console.log(`cart-service running on http://0.0.0.0:${PORT}`);
});
