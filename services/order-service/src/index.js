...
app.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    try {
      ...
      let total = 0;
      for (const item of cartItems) total += Number(item.price) * Number(item.quantity);
      ...
      const [orders] = await conn.execute("SELECT id, user_id, total_price, created_at FROM orders WHERE id=?", [
        orderId
      ]);
    ...
    try {
      const [orders] =
        userId && Number.isInteger(userId)
          ...
          : await conn.execute("SELECT id, user_id, total_price, created_at FROM orders ORDER BY created_at DESC");
...
app.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const conn = await pool.getConnection();
    try {
      const [orders] = await conn.execute("SELECT id, user_id, total_price, created_at FROM orders WHERE id=?", [id]);
